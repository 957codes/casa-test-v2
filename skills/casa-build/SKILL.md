...
3. Do the work to a quality bar. Follow the procedure and produce the real artifact,
   not a placeholder. If the playbook produces founder-facing copy, draft it through
   `casa-write` (which runs the deterministic copy linter). If it produces UI, use
   `casa-design`. Keep the company profile and any locked decisions in view.
